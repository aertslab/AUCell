...
```
The threshold to calculate the AUC, is determined by *aucMaxRank*. By default, it is set to 5% of the number of genes in the ranking (i.e. checks whether the genes in the gene-set or signature are within the top 5%). However, for datasets where most cells (or "samples") express many genes, or these have high expression values, it might be good to increase this threshold. Check the histogram provided by `AUCell_buildRankings` to get an estimation on where this threshold lies within the dataset.
...
AUCell_plot(cells_AUC[geneSetName,])
abline(v=0.08)
```
...
plot(cellTsne$Y, main=paste(geneSetName, "\n- new assignment"),
       col=c("#90909030","blue")[as.numeric(getAUC(cells_AUC)[geneSetName,rownames(cellTsne$Y)]>newThreshold)+1],
       pch=16)
```
...
plot(cellTsne$Y, axes=FALSE, xlab="", ylab="",
       sub="Number of detected genes",
       col=cellColorNgenes[rownames(cellTsne$Y)], pch=16)
```
...
for(geneSetName in names(geneSets))
{
  ...
  plot(cellTsne$Y, main=geneSetName, axes=FALSE, xlab="", ylab="",
       sub="Expression mean",
       col=cellColor[rownames(cellTsne$Y)], pch=16)
}
```
...
Confusion matrix (cells-signature assigned by AUCell *vs* author's cell-type label):

*Note* on interpretation: Interneurons & pyramidal are "neurons". According to other publications that have used this dataset to evaluate new methods, a relevant group of cells marked as oligodendrocytes are likely doublets (i.e. two cells, for example a mix of an oligodendrocyte + another cell type).
```{r}
cellTypeNames <- unique(cellLabels[,"level1class"])
confMatrix <- t(sapply(cells_assignment[c(1,4,2,5,3,6:9)], function(x) table(cellLabels[x$assignment,])[cellTypeNames]))
